...
Page({
  ...
  onLoad: function (options) {
    ...
    //grade_active
    for(var i=0;i<that.data.gradeArr.length;i++){
    	that.data.grade_active[i]='';
    }
    that.setData({grade_active:that.data.grade_active})
    //从二维码到分类列表页
        ...
        function (res) {
          if (res.data.length > 0) {
            ...
            for (var i = 0; i < res.data.length; i++) {
              var bookPic = res.data[i].book_pic;
              res.data[i].book_pic = bookPic.substring(0, bookPic.length - 4) + "_c" + bookPic.substring(bookPic.length - 4)
