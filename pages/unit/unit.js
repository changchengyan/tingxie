...
Page({
  ...
  sendWords: function (e) {
    var that = this;
    console.log(typeof that.data.user_input);
    //确认input输入长度
    if (that.data.user_input.length == 0) {
      wx.showToast({
        title: '单词处于编辑中，或输入为空',
        duration: 1000
      })
  ...
  addShare: function (e) {
    ...
    var index = e.currentTarget.dataset.index;
    var newWords = this.data.words;
    newWords.push(this.data.share_words[index]);
    this.setData({
      words: newWords
    })
    ...
    if (newShareWords.length === 0) {
      ...
      that.setData({
        share_words: newShareWords
      })
    }
    this.getShowWords(this.data.words);
    this.getShareShowWords(this.data.share_words);
    this.wordsCount(this.data.words);
  },
  //点击叉删除词语
